#!/usr/bin/env python3
"""Fine-tune MiniCPM5 2B for VoiceInk transcription cleanup with Unsloth.
...
DEFAULT_TRAIN = ROOT / "datasets" / "releases" / "voiceink-data-v1" / "train.jsonl"
DEFAULT_EVAL = ROOT / "datasets" / "releases" / "voiceink-data-v1" / "validation.jsonl"
...
MINICPM5_2B_PROFILE = TrainingProfile(
    name="MiniCPM5 2B",
    description="Fine-tune MiniCPM5 2B for VoiceInk with Unsloth.",
...
def parse_args(
    profile: TrainingProfile = MINICPM5_2B_PROFILE,
        ...
        for line_number, line in enumerate(stream, 1):
            ...
            for message in messages:
                ...
                if not isinstance(content, str) or not content.strip():
                    raise ValueError(
                        f"{path}:{line_number}: MiniCPM5 requires non-empty string content"
                    )
...
def print_preflight(args: argparse.Namespace, train_data: list[dict],
    ...
    print("MiniCPM5 training preflight")
...
def main(
    ...
    tracking = start_mlflow_run(
        ...
        run_kind="training.sft.minicpm5",
        params={
    ...
    tracking.log_external_model(
        ...
        tags={"voiceink.training_family": "minicpm5-sft"},
        metrics={
            **stats.metrics,
