...
Every model hit the token cap at 4,000 tokens — none of them stopped naturally. The repetition counts were similar across 4B, 9B, and 35B (the coaching phrases genuinely repeat in the input). But the fine-tuned 2B uniquely amplified the repetition to fill the entire 16K window.
...
**Labels matter more than model size.** The 9B base model scored 82.9, the 4B scored 82.3 — effectively identical. A fine-tuned 2B with good labels scored 92.1, beating both by 8-10 points. If your labels are mediocre, throwing a bigger model at the problem won't help.

**Completions-only training is critical for chat tasks.** When the system prompt contains dynamic context (clipboard, active window, vocabulary) that changes every request, training the model to predict those tokens is wasted capacity. Masking loss on everything except the assistant response let the 2B model focus entirely on the actual task.

**Your training data distribution defines your model's capabilities.** If 99.7% of training data is short dictation, the model will excel at short dictation and fail at everything else. Four long samples out of 1,175 weren't just underrepresented — they were statistically invisible. The model literally had no examples of how to handle long, repetitive input.

**Synthetic data works for tail cases.** 120 synthetic samples, generated by Claude Sonnet 4.6 with carefully designed prompts matching real production patterns, completely fixed a production-breaking bug. The key was fidelity — using the exact coaching phrase structures, STT error patterns, and speaker characteristics from real data.

**Log everything in production.** The passive JSONL logging proxy gave me 1,175 real-world training samples with zero manual effort. No labeling UI, no annotation workflow, no data collection campaign. Just a reverse proxy that writes every request/response pair to a file.

**LLM-as-judge scales across the pipeline.** Claude Sonnet 4.6 served as labeler (generating gold-standard training labels), evaluator (blind A/B scoring with a calibrated rubric), and synthetic data generator (producing matched transcript/label pairs). The entire pipeline — from raw logs to deployed model — used no manual annotation at any point.

## The Stack

- **Base model**: [Qwen 3.5 2B](https://huggingface.co/Qwen/Qwen3.5-2B) (unified VLM architecture)
- **Training**: [Unsloth](https://unsloth.ai/) ([GitHub](https://github.com/unslothai/unsloth)) with LoRA, on a consumer RTX 4080 Super (16GB VRAM)
- **Inference**: [llama.cpp](https://github.com/ggerganov/llama.cpp) (`llama-server`) serving Q4_K_M GGUF
- **Labeling and evaluation**: Claude Sonnet 4.6 as LLM-as-judge
- **Synthetic data**: Claude Sonnet 4.6 generating matched transcript/label pairs
- **Development**: [Claude Code](https://claude.ai/claude-code) for all code, planning, and debugging
- **Dictation**: [VoiceInk](https://voiceink.app) with Parakeet V2 STT engine
...
# Fine-tune and export to GGUF
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  .venv/bin/python3 src/training/finetune.py --epochs 2 --export-gguf q4_k_m

# Evaluate
python3 src/eval/evaluate.py --baseline Qwen3.5-4B --candidate Qwen3.5-2B-VoiceInk
```

The model is running in production now, cleaning up every dictation I send through VoiceInk — including the ones I used to draft parts of this post.
